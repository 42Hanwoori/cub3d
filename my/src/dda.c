...
{
	ray->y_intercept = floor(set->player.y / TILE_SIZE) * TILE_SIZE\
	+ (ray->ray_facing_down * TILE_SIZE);
	ray->x_intercept = set->player.x + (ray->y_intercept -\
	set->player.y) / tan(ray->ray_angle);
	ray->y_step = TILE_SIZE;
	if (ray->ray_facing_up)
		ray->y_step *= -1;
	ray->x_step = TILE_SIZE / tan(ray->ray_angle);
	if ((ray->ray_facing_left && ray->x_step > 0) || (ray->ray_facing_right\
	&& ray->x_step < 0))
		ray->x_step *= -1;
}

void	check_ray_hit_horz(t_set *set, t_ray *ray, double *x, double *y)
{
	double	next_horz_x;
	double	next_horz_y;
	double	y_to_check;
	...
	while (next_horz_x >= 0 && next_horz_x <= set->map_width * TILE_SIZE &&
	next_horz_y >= 0 && next_horz_y <= set->map_height * TILE_SIZE)
	{
		y_to_check = ray->ray_facing_up ? next_horz_y - 1 : next_horz_y;
...
void	check_ray_hit_vert_set(t_set *set, t_ray *ray)
{
	ray->wall_hit_x = 0;
	ray->wall_hit_y = 0;
	ray->x_intercept = floor(set->player.x / TILE_SIZE) * TILE_SIZE +\
	(ray->ray_facing_right * TILE_SIZE);
	ray->y_intercept = set->player.y + ((ray->x_intercept - set->player.x)\
	* tan(ray->ray_angle));
...
void	check_ray_hit_vert(t_set *set, t_ray *ray, double *x, double *y)
{
	double	next_vert_x;
	double	next_vert_y;
	double	x_to_check;
	...
	while (next_vert_x >= 0 && next_vert_x <= set->map_width * TILE_SIZE &&
	next_vert_y >= 0 && next_vert_y <= set->map_height * TILE_SIZE)
	{
		x_to_check = ray->ray_facing_left ? next_vert_x - 1 : next_vert_x;
...
void	ray_cal_dda(t_set *set, t_ray *ray, double horz_x, double horz_y)
{
	double	vert_x;
	double	vert_y;
	...
	ray->h_dist = dbt(horz_x, horz_y, set->player.x, set->player.y);
	ray->h_dist = (ray->found_horz_wall_hit != 1) ? F_MAX : ray->h_dist;
	ray->v_dist = dbt(vert_x, vert_y, set->player.x, set->player.y);
