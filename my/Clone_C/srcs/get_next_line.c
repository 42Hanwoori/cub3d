...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/11/13 17:23:30 by ji-park           #+#    #+#             */
/*   Updated: 2020/11/14 18:02:08 by ji-park          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		separate(char **line, char **storage, int cut_idx)
{
	...
	(*storage)[cut_idx] = 0;
	remain_len = ft_strlen2(*storage + cut_idx + 1);
	*line = ft_strdup2(*storage);
	if (remain_len == 0)
	{
		free(*storage);
		*storage = 0;
	...
	temp = ft_strdup2(*storage + cut_idx + 1);
	free(*storage);
	*storage = temp;
	return (1);
}
...
int		get_next_line(int fd, char **line)
{
	...
	{
		if (storage[fd] == 0)
			storage[fd] = ft_strdup2("");
		buf[read_size] = '\0';
		storage[fd] = ft_strjoin2(storage[fd], buf);
